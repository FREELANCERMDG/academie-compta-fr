...
<style>
/* ===== Amelioration du design (page de cours) — surcouche non destructive ===== */
:root{--bg:#eef2f7;--muted:#6a7889;--bd:#e4e9f0;--bd2:#eef2f7;
  --shadow-sm:0 1px 2px rgba(20,40,70,.06);
  --shadow:0 6px 22px rgba(20,40,70,.07),0 1px 3px rgba(20,40,70,.06);}
html{scroll-behavior:smooth}
body{background:radial-gradient(1200px 420px at 100% -8%,rgba(46,108,164,.06),transparent),var(--bg);-webkit-font-smoothing:antialiased;font-size:15.5px}

/* Header */
header{background:linear-gradient(120deg,var(--navy),var(--navy2));border-bottom:1px solid rgba(0,0,0,.06)}
header .logo{letter-spacing:.2px}
#search{border:1px solid rgba(255,255,255,.45);background:rgba(255,255,255,.96);font-size:13.5px;transition:box-shadow .15s,width .2s}
#search:focus{outline:none;box-shadow:0 0 0 3px rgba(232,161,58,.45)}
#searchgo{margin-left:6px;padding:7px 12px;border:none;border-radius:8px;background:var(--accent);color:#1c2733;font-weight:700;font-size:13px;cursor:pointer;white-space:nowrap}
#searchgo:hover{filter:brightness(.96)}
.content mark.shl{background:#fff2a8;color:#16263a;padding:0 2px;border-radius:3px;box-shadow:0 0 0 2px #ffe066}
.progwrap{color:rgba(255,255,255,.92)}
.progbar{height:9px}
.progbar i{transition:width .45s ease;background:linear-gradient(90deg,var(--accent),#f6c172)}

/* Sidebar */
.sidebar{background:linear-gradient(180deg,#fff,#fcfdff);scrollbar-width:thin;scrollbar-color:#c7d3e2 transparent}
.sidebar::-webkit-scrollbar{width:9px}
.sidebar::-webkit-scrollbar-thumb{background:#cdd9e8;border-radius:8px;border:2px solid #fff}
.sidebar .grp{position:sticky;top:0;z-index:1;background:rgba(255,255,255,.97);padding:13px 16px 7px;color:#94a2b3;border-bottom:1px solid var(--bd2)}
.sidebar a.item{font-size:13.5px;padding:9px 14px;margin:1px 8px 1px 0;border-radius:0 999px 999px 0;transition:background .12s,color .12s}
.sidebar a.item:hover{background:#eef4fb}
.sidebar a.item.active{background:linear-gradient(90deg,#eaf2fb,#f6f9fd);border-left-color:var(--accent);box-shadow:inset 0 0 0 1px #dbe7f5}
.sidebar a.item.active .dot{border-color:var(--accent)}
.sidebar a.item.done{color:#3a6b52}

/* Contenu */
.main{padding-top:30px}
.content.card{border:1px solid var(--bd);box-shadow:var(--shadow);border-radius:18px;padding:30px 38px}
.content{font-size:15.5px;color:#243240}
.content>*:first-child{margin-top:0}
.content :where(h1,h2,h3,h4){scroll-margin-top:76px;line-height:1.25}
.content h1{font-size:27px;letter-spacing:-.2px;border-bottom:none;position:relative;margin-bottom:18px;padding-bottom:14px}
.content h1::after{content:"";position:absolute;left:0;bottom:0;width:66px;height:4px;border-radius:3px;background:linear-gradient(90deg,var(--accent),#f6c172)}
.content h2{font-size:20.5px;margin-top:34px;padding-left:13px;border-left:4px solid var(--accent)}
.content h3{font-size:16.5px;margin-top:24px}
.content p,.content ul,.content ol{max-width:76ch}
.content li{margin:5px 0}
.content a{color:var(--navy2);text-decoration:none;border-bottom:1px solid rgba(46,108,164,.32)}
.content a:hover{border-bottom-color:var(--navy2)}
.content strong{color:#16263a}
.content blockquote{background:linear-gradient(180deg,#f1f6fc,#eef4fb);border-left:4px solid var(--navy2);border-radius:10px;padding:12px 16px;box-shadow:var(--shadow-sm)}
.content code{background:#eef2f7;border:1px solid #e2e8f1;color:#1c3d5e}
...
.btn.sec{background:#fff;border:1px solid var(--bd);color:var(--navy)}
.pagebar{position:sticky;bottom:0;justify-content:space-between;background:linear-gradient(180deg,rgba(238,242,247,0),var(--bg) 45%);backdrop-filter:blur(3px);padding-bottom:14px;margin-top:30px}

/* Quiz */
.quiz{box-shadow:var(--shadow);border-radius:16px}

@media(max-width:860px){
  .content.card{padding:20px 18px;border-radius:14px}
  .content h1{font-size:23px}
  .main{padding-top:18px}
  .sidebar{box-shadow:6px 0 30px rgba(10,25,50,.25)}
}

/* ============================================================
   THEME FUTURISTE (override sombre) — coque sombre/neon + feuille de cours lumineuse
   ============================================================ */
:root{--neon:#38e8ff;--neon2:#7c6cff}
body{background:
    radial-gradient(1100px 600px at 88% -6%, rgba(124,108,255,.22), transparent 60%),
    radial-gradient(900px 500px at -6% 8%, rgba(56,232,255,.12), transparent 55%),
    linear-gradient(160deg,#0b0f24,#141a3a 55%,#1b1247) !important;
  background-attachment:fixed;color:#e6ecfa}
body::before{content:"";position:fixed;inset:0;z-index:0;pointer-events:none;
  ...
  -webkit-mask-image:radial-gradient(1200px 820px at 50% 0,#000 30%,transparent 86%);
  mask-image:radial-gradient(1200px 820px at 50% 0,#000 30%,transparent 86%)}
header,.layout{position:relative;z-index:1}

/* Barre du haut : verre sombre + neon */
header{background:rgba(10,13,32,.72) !important;backdrop-filter:blur(14px);-webkit-backdrop-filter:blur(14px);border-bottom:1px solid rgba(56,232,255,.18) !important;box-shadow:0 10px 32px -14px rgba(56,232,255,.40)}
header .logo{color:#fff;text-shadow:0 0 16px rgba(56,232,255,.40)}
header .logo span{color:#aab4d4}
#search{background:rgba(255,255,255,.08) !important;border:1px solid rgba(56,232,255,.28) !important;color:#eaf0fb}
#search::placeholder{color:#8e9bbd}
#search:focus{box-shadow:0 0 0 3px rgba(56,232,255,.30) !important}
.progbar{background:rgba(255,255,255,.12)}
.progbar i{background:linear-gradient(90deg,#38e8ff,#7c6cff) !important;box-shadow:0 0 12px rgba(56,232,255,.6)}
.progwrap{color:#cdd6ee}

/* Sommaire (sidebar) : verre sombre */
.sidebar{background:linear-gradient(180deg,rgba(255,255,255,.045),rgba(255,255,255,.02)) !important;border-right:1px solid rgba(56,232,255,.12);backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px)}
.sidebar .grp{background:rgba(13,18,42,.94) !important;color:#8ea3c9 !important;border-bottom:1px solid rgba(255,255,255,.06)}
.sidebar a.item{color:#cdd6ee !important}
.sidebar a.item:hover{background:rgba(56,232,255,.08) !important}
.sidebar a.item.active{background:linear-gradient(90deg,rgba(56,232,255,.16),rgba(124,108,255,.06)) !important;box-shadow:inset 0 0 0 1px rgba(56,232,255,.30) !important;color:#fff !important;border-left-color:var(--neon) !important}
.sidebar a.item .dot{border-color:#52608a}
.sidebar a.item.done{color:#7ee0aa !important}
.sidebar a.item.done .dot{background:#34d399;border-color:#34d399}
.sidebar::-webkit-scrollbar-thumb{background:rgba(124,170,255,.30);border:2px solid transparent}
.crumbs{color:#8ea3c9 !important}

/* Feuille de cours EN SOMBRE + prose ; widgets/documents (CERFA, simulateurs, ecritures) = ilots clairs proteges */
.content.card{background:rgba(16,22,44,.88) !important;color:#dfe7f5 !important;border:1px solid rgba(56,232,255,.28) !important;
  box-shadow:0 26px 64px rgba(0,0,0,.50),0 0 0 1px rgba(56,232,255,.10),0 0 46px -14px rgba(56,232,255,.36) !important}
.content{color:#dfe7f5}
.content h1,.content h2{color:#fff}
.content h3{color:#bcd0ff}.content h4{color:#cdd9ef}
.content strong{color:#fff}
.content a:not([class]){color:#8fd9ff;border-bottom-color:rgba(143,217,255,.40)}
.content a:not([class]):hover{border-bottom-color:#8fd9ff}
.content blockquote{background:rgba(56,232,255,.07) !important;border-left-color:var(--neon) !important;color:#e7eefb !important;box-shadow:none !important}
.content code{background:rgba(255,255,255,.08) !important;border-color:rgba(255,255,255,.14) !important;color:#ffd9a8 !important}
.content hr{border-top-color:rgba(255,255,255,.12)}
.content .tbl{border-color:rgba(255,255,255,.12)}
.content tbody tr:nth-child(even){background:rgba(255,255,255,.04)}
.content tbody tr:hover{background:rgba(56,232,255,.09)}
.content td{border-color:rgba(255,255,255,.12)}
.content details.corrige{background:rgba(232,161,58,.10) !important;border-color:rgba(232,161,58,.42) !important}
.content details.corrige>summary{background:linear-gradient(180deg,rgba(232,161,58,.20),rgba(232,161,58,.10)) !important;color:#ffce8a !important;border-bottom-color:rgba(232,161,58,.30) !important}
.content .corrige-seuil{color:#cdd9ef}
.content mark.shl{color:#16263a}
/* Quiz en sombre */
.quiz{background:rgba(16,22,44,.88) !important;color:#dfe7f5 !important;border:1px solid rgba(56,232,255,.25) !important;box-shadow:0 18px 48px rgba(0,0,0,.46),0 0 30px -12px rgba(56,232,255,.30) !important}
.quiz h3{color:#fff !important}
.q{border-color:rgba(255,255,255,.12) !important;background:rgba(255,255,255,.03) !important}
.q .opt{border-color:rgba(255,255,255,.14) !important;background:rgba(255,255,255,.045) !important;color:#dfe7f5}
.q .opt:hover{background:rgba(56,232,255,.10) !important}
.q .opt.correct{background:rgba(52,211,153,.18) !important;border-color:#34d399 !important}
.q .opt.wrong{background:rgba(248,113,113,.16) !important;border-color:#f87171 !important}
.q .exp{color:#bcd0ff !important}
.score{color:#fff}
/* ILOTS CLAIRS : documents/logiciels reels gardent leur fond clair + texte sombre */
.content .simdoc, .content .sc, .content .cerfa-doc, .content table.ecr, .content .pj{color:#1c2733}
/* on empeche le striping/hover/bordures sombres de fuiter dans les tableaux des simulateurs & ecritures */
.content :is(.sc,.simdoc,table.ecr) tbody tr:nth-child(even){background:transparent !important}
.content :is(.sc,.simdoc,table.ecr) tbody tr:hover{background:rgba(20,40,70,.05) !important}
.content :is(.sc,.simdoc) td{border-color:#e6ecf7 !important}

/* Barre de navigation bas : fondu sombre */
.pagebar{background:linear-gradient(180deg,rgba(11,15,36,0),rgba(11,15,36,.88) 45%) !important;border-top-color:rgba(56,232,255,.15) !important}
.btn.sec{background:rgba(255,255,255,.06) !important;color:#e7ecfa !important;border:1px solid rgba(56,232,255,.25) !important}
.btn.sec:hover{background:rgba(255,255,255,.12) !important}

@media(max-width:860px){.sidebar{background:#0d1330 !important}}
...
function show(id){
  ...
  const _act=document.querySelector('.sidebar a.item.active');if(_act&&_act.scrollIntoView)_act.scrollIntoView({block:'nearest'});
  // boutons prev/next
  ...
  if(window.__pendingHL){const t=window.__pendingHL;window.__pendingHL=null;highlightInContent(t);}
}
...
function highlightInContent(term){
  const root=document.getElementById('content');if(!root||!term)return;
  const t=term.toLowerCase();
  const w=document.createTreeWalker(root,NodeFilter.SHOW_TEXT,null);const nodes=[];let n;while(n=w.nextNode())nodes.push(n);
  for(const node of nodes){
    const txt=node.nodeValue;if(!txt)continue;
    const p=node.parentNode;if(p&&(p.tagName==='SCRIPT'||p.tagName==='STYLE'))continue;
    const idx=txt.toLowerCase().indexOf(t);if(idx<0)continue;
    const span=document.createElement('span');
    span.appendChild(document.createTextNode(txt.slice(0,idx)));
    const mk=document.createElement('mark');mk.className='shl';mk.textContent=txt.slice(idx,idx+term.length);
    span.appendChild(mk);span.appendChild(document.createTextNode(txt.slice(idx+term.length)));
...
function goSearch(){
  const inp=document.getElementById('search');const raw=(inp?inp.value:'').trim();const q=norm(raw);
  if(!q)return;
  if(!SEARCHIDX)buildIdx();
  let id=null;for(const x of ORDER){if((SEARCHIDX[x]||'').includes(q)){id=x;break;}}
  if(!id){applySearch();return;}
  window.__pendingHL=raw;
  if(curId()===id){show(id);}else{location.hash=id;}
...
document.getElementById('search').addEventListener('keydown',function(e){if(e.key==='Enter'){e.preventDefault();goSearch();}});
