...
export function openDB() {
  ...
  // Visiteurs UNIQUES : on ne compte chaque IP (hachée) qu'une seule fois — amorcé avec les IP déjà connues
  try { db.exec("CREATE TABLE IF NOT EXISTS visites_seen(k TEXT PRIMARY KEY, jour TEXT)"); } catch { }
  try { db.exec("INSERT OR IGNORE INTO visites_seen(k,jour) SELECT k, substr(COALESCE(ts,''),1,10) FROM ip_region"); } catch { }
