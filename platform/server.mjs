...
function layout(title, body, sess) {
  ...
  const nav = u
    ? `<a href="/programme">Programme</a><a href="/presentiel">Présentiel</a><a href="/emploi">Emploi</a><a href="/communaute">Communauté</a><a href="/cabinet">🏢 Cabinet</a><a href="/logiciel">🧪 Logiciel</a><a href="/formation">📚 Formation</a><a href="/tableau-de-bord">Mon espace</a>${u.role === 'admin' ? '<a href="/admin">Admin</a>' : ''}<a href="/deconnexion">Déconnexion</a>`
...
<body><div class="site-bg" aria-hidden="true"></div><div class="topbar"><header class="top"><a class="brand" href="/">${esc(cfg.site.nom_plateforme)}</a><input type="checkbox" id="navtog" class="navtog" aria-hidden="true"><label for="navtog" class="navbtn" aria-label="Ouvrir le menu"><span></span><span></span><span></span></label><nav id="navmenu">${nav}</nav></header>${promoBan}
${ticker}
...
function apercuModulesSection() {
  ...
  <p style="text-align:center;color:#d7e3ee;font-size:13.5px;margin:14px 0 0;padding:11px 15px;background:rgba(30,125,70,.14);border:1px solid rgba(110,231,168,.45);border-radius:12px">🎓 <b style="color:#fff">Attestation à la clé</b> — dès que vous maîtrisez <b style="color:#fff">les bases jusqu'à la fiscalité</b>, obtenez votre attestation après un court <b style="color:#fff">test final en visio</b> avec le formateur (signée &amp; tamponnée).</p>
  <p style="text-align:center;color:#d7e3ee;font-size:13.5px;margin:10px 0 0;padding:11px 15px;background:rgba(124,108,255,.14);border:1px solid rgba(124,108,255,.5);border-radius:12px">🎯 <b style="color:#fff">Orientation &amp; accompagnement sur les débouchés</b> — on ne s'arrête pas à la technique : vous êtes guidé(e) sur vos <b style="color:#fff">débouchés</b> (collaborateur pour cabinets français depuis Madagascar, <b style="color:#fff">freelance payé en euros</b>), avec préparation aux entretiens et accès à notre <a href="/emploi" style="color:#8fd6ff;font-weight:700">bourse d'offres d'emploi</a>.</p></section>`;
}
...
function pageAdmin(sess, notif, acces, accesEmail, qRaw) {
  ...
  <section class="card"><details class="fold"${acces === 'att_ok' || acces === 'att_off' ? ' open' : ''}><summary>👥 Apprenants (${users.length})</summary>
