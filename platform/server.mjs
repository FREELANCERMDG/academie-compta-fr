...
async function postApiChat(req, res, body) {
  ...
  try {
    ...
    if (!r.ok) { let etype = ''; try { const ed = await r.json(); etype = (ed && ed.error && (ed.error.message || ed.error.type)) || ''; } catch { } try { audit(db, null, 'chat_api_err', 'http ' + r.status + ' ' + etype, ip(req)); } catch { } return J({ error: true, status: r.status, type: String(etype).slice(0, 180), model: model }); }
  ...
  } catch (e) { return J({ error: true, ex: String((e && e.name) || e).slice(0, 60) }); }
}
