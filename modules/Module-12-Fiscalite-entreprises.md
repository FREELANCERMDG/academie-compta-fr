# MODULE 12 — Fiscalité des entreprises françaises

**Niveau :** Avancé · **Durée estimée :** 10–12 h · **Prérequis :** Modules 5 et 11
**Chiffres à jour 2026** (sources : impots.gouv.fr, bofip.impots.gouv.fr, economie.gouv.fr).

> ⚠️ **Posture offshore (rappel essentiel) :** le collaborateur **prépare et rassemble** les éléments fiscaux. Le **calcul définitif, la validation et la signature** des déclarations relèvent de l'expert-comptable. On ne « décide » jamais un traitement fiscal seul.

---

## 🎯 Objectifs pédagogiques
- Distinguer IR / IS, BIC / BNC / BA, micro / réel.
- Connaître les déclarations (2031, 2035, 2065) et la liasse fiscale.
- Comprendre acomptes/solde IS, CFE, CVAE, taxe sur les salaires, taxe sur les véhicules.
- Connaître les principales échéances.

---

## 1. Qui paie quoi : IR ou IS ?
| | **Impôt sur le revenu (IR)** | **Impôt sur les sociétés (IS)** |
|---|---|---|
| Qui | Entreprises individuelles, EURL (par défaut), certaines sociétés sur option | SARL, SAS, SA, EURL sur option… |
| Imposition | Le bénéfice est imposé au nom de l'**exploitant/associé** (barème IR) | La **société** paie l'impôt sur son bénéfice |
| Déclaration de résultat | **2031** (BIC) / **2035** (BNC) | **2065** |

---

## 2. Les catégories de revenus professionnels
| Catégorie | Activité | Régimes |
|---|---|---|
| **BIC** (Bénéfices industriels et commerciaux) | Commerce, artisanat, industrie | Micro-BIC, réel simplifié, réel normal |
| **BNC** (Bénéfices non commerciaux) | Professions libérales, prestations intellectuelles | Micro-BNC, déclaration contrôlée (réel) |
| **BA** (Bénéfices agricoles) | Agriculture | Micro-BA, réel |

---

## 3. Les régimes d'imposition du bénéfice (seuils 2026)
| Régime | Seuils de CA (2026) | Abattement / obligations |
|---|---|---|
| **Micro-BIC vente** | ≤ **203 100 €** | Abattement **71 %** ; pas de bilan |
| **Micro-BIC services** | ≤ **83 600 €** | Abattement **50 %** |
| **Micro-BNC** | ≤ **83 600 €** | Abattement **34 %** (déclaration 2042-C-PRO) |
| **Réel simplifié (BIC)** | jusqu'aux seuils du réel normal | Liasse 2033 (tableaux simplifiés) |
| **Réel normal (BIC)** | au-delà / sur option | Liasse 2050 et suivants |
| **Déclaration contrôlée (BNC)** | sur option / au-delà du micro | **2035** |

---

## 4. L'impôt sur les sociétés (IS) — taux 2026
- **Taux normal : 25 %**.
- **Taux réduit : 15 %** sur les **premiers 42 500 € de bénéfice**, pour les PME dont :
  - le **CA HT < 10 M€**,
  - le **capital entièrement libéré**,
  - **détenu à ≥ 75 % par des personnes physiques** (ou sociétés répondant elles-mêmes aux conditions).
- Au-delà de 42 500 € : taux normal de 25 %.

**Ex. — Bénéfice 60 000 € (PME éligible) :**
- 42 500 × 15 % = 6 375 €
- (60 000 − 42 500) × 25 % = 17 500 × 25 % = 4 375 €
- **IS = 10 750 €**

### Acomptes et solde IS
- **4 acomptes** (formulaire **2571**) : **15/03, 15/06, 15/09, 15/12**. Chaque acompte = **25 % de l'IS de référence** (IS du dernier exercice clos).
- **Solde** (formulaire **2572**) : **15/05/N** pour un exercice clos le 31/12/N‑1 (sinon le 15 du 4e mois suivant la clôture).

**⭐ Régularisation au 2e acompte (point clé)**
Au **1er acompte (15/03)**, le résultat **N‑1 n'est pas encore connu** (la liasse n'est déposée qu'en mai) → on le calcule sur l'**avant‑dernier exercice (N‑2)**.
Au **2e acompte (15/06)**, le résultat **N‑1 est connu** → on **régularise** : le 2e acompte « rattrape » le 1er pour que **le cumul des 2 premiers acomptes = la moitié (2/4) de l'IS N‑1**.
> **Formule :** **2e acompte = (IS N‑1 × 2/4) − 1er acompte déjà versé.**
Les **3e et 4e acomptes** = **25 % de l'IS N‑1** chacun. Le **solde** (15/05/N+1) régularise le tout selon l'**IS réel de N**.

**Exemple :** IS N‑2 = 8 000 € ; IS N‑1 (connu en mai) = 12 000 €.
- 1er acompte (15/03) = 8 000 × 25 % = **2 000 €** (calculé sur N‑2).
- **2e acompte (15/06) régularisé** = (12 000 × 2/4) − 2 000 = 6 000 − 2 000 = **4 000 €**.
- 3e (15/09) = 12 000 × 25 % = **3 000 €** · 4e (15/12) = **3 000 €**.
- Total acomptes N = 12 000 € ; **solde** = IS réel N − 12 000 €.
> 💡 Le collaborateur **prépare** le calcul (2571) ; l'**expert‑comptable valide**. Un acompte peut être **modulé/réduit** si le résultat baisse (sous conditions, décision de l'EC). Pas d'acompte la **1re année** d'existence, ni si l'IS de référence < 3 000 €.

---

## 5. La liasse fiscale
La **liasse fiscale** = ensemble des tableaux (bilan, compte de résultat, annexes fiscales) transmis à l'administration avec la déclaration de résultat. Selon le régime : **2031/2033** (BIC RSI), **2050-2059** (BIC réel normal), **2035** (BNC), **2065** (IS).
- **Date limite de dépôt** : généralement **début mai** (2e jour ouvré suivant le 1er mai) pour un exercice clos au 31/12, avec la **télétransmission EDI-TDFC** (délai supplémentaire de 15 jours en pratique).

> Rôle du collaborateur : **préparer les tableaux, justifier les comptes, rassembler les éléments** ; l'expert-comptable **finalise et télétransmet**.

---

## 5 bis. Les formulaires fiscaux (CERFA) — case par case

> ⚠️ En pratique, la **liasse est générée par le logiciel** (RCA, ACD, Cegid…) à partir de la compta révisée : le collaborateur **prépare et contrôle**, l'**expert-comptable valide et télétransmet** (EDI-TDFC). Voici, pour comprendre **où va quoi**, les cadres et lignes **clés** de chaque formulaire.

### 🧾 2065‑SD — Impôt sur les sociétés (IS)
Formulaire court : le **résultat** vient des **tableaux de la liasse**, et le **calcul de l'IS** se fait sur d'autres imprimés.

<div class="cerfa" data-form="2065"></div>
| Cadre / ligne | Quoi y mettre |
|---|---|
| En‑tête | Identité, **SIRET**, exercice, régime (réel normal / simplifié) |
| **A — Récapitulation** | Report du **résultat fiscal** issu du tableau **2058‑A** (réel normal) ou **2033‑B** (réel simplifié) |
| **C‑1 — Résultat fiscal** | **Bénéfice imposable** (ou **déficit**) de l'exercice |
| Plus/moins‑values | À long terme (taux réduit) le cas échéant |
| **2065 bis** | Répartition du bénéfice, **rémunérations des dirigeants**, divers |
> Le **montant de l'IS** (15 %/25 %) n'est PAS sur la 2065 : il se calcule sur le **2571** (acomptes) et le **2572** (solde).

### 🧾 2031‑SD — BIC à l'IR (entreprise individuelle, EURL à l'IR…)
Même logique, mais pour une entreprise **à l'IR** :

<div class="cerfa" data-form="2031"></div>
| Cadre | Quoi y mettre |
|---|---|
| En‑tête | Identité, SIRET, régime |
| Résultat | **Bénéfice (ou déficit) BIC** issu de **2050‑2059** (réel normal) ou **2033** (réel simplifié) |
| Répartition | Quote‑part des associés (sociétés de personnes) |
> Le résultat est ensuite **reporté sur la déclaration de revenus** du dirigeant (**2042‑C‑PRO**).

### 🧾 2035‑SD — BNC (déclaration contrôlée) — comptabilité de **caisse**
Le plus « manuel » des trois : **2035‑A** (résultat) + **2035‑B** (résultat fiscal).
**2035‑A — Recettes & dépenses :**
| Poste | Quoi y mettre |
|---|---|
| **Recettes** | **Recettes encaissées** (honoraires…) — pas les factures impayées |
| Débours / rétrocessions | Sommes reversées à des confrères |
| **Dépenses** | **payées** dans l'année : achats, **loyer (6132)**, **véhicule/déplacements**, fournitures, **honoraires**, **cotisations sociales TNS**, télécom, assurances… |
| Amortissements | Dotations (tableau des immobilisations) |
**2035‑B — Résultat :**
| Ligne | Quoi y mettre |
|---|---|
| Excédent / Insuffisance | **Bénéfice ou déficit BNC** |
| CSG déductible, divers | Régularisations |
| Plus‑values | Le cas échéant |
> BNC = **recettes encaissées** et **dépenses payées**. Résultat reporté sur **2042‑C‑PRO**.

### 🧾 CFE — 1447‑C‑SD (création) / 1447‑M‑SD (modification)
La CFE est **calculée par l'administration** (avis reçu en ligne, paiement le **15/12**). On ne **dépose une déclaration** que dans 2 cas :

<div class="cerfa" data-form="cfe"></div>
| Formulaire | Quand | Cases clés |
|---|---|---|
| **1447‑C‑SD** | l'**année de création** (avant le 1er janvier suivant) | identité, **adresse des locaux**, **surfaces / valeur locative**, effectif, exonérations |
| **1447‑M‑SD** | en cas de **changement** (surface, activité, demande d'exonération) | uniquement les éléments modifiés |
> 1re année : **exonération de CFE**. Ensuite : avis en ligne, paiement 15/12 (acompte en juin si CFE N‑1 ≥ 3 000 €).

### 📎 Formulaires officiels
2065‑SD · 2031‑SD · 2035‑SD (+ 2035‑A/B) · 1447‑C‑SD / 1447‑M‑SD — tous sur **impots.gouv.fr** (« Recherche de formulaires »). Tableaux de liasse : **2050‑2059** (réel normal), **2033‑A à 2033‑G** (réel simplifié).

---

## 6. Autres impôts et taxes

### CFE — Cotisation foncière des entreprises
Due par toute entreprise (sauf 1re année / exonérations) ; basée sur la valeur locative des locaux. **Échéance : 15 décembre** (acompte en juin si CFE N-1 ≥ 3 000 €).

### CVAE — Cotisation sur la valeur ajoutée des entreprises (2026)
- Due si **CA > 500 000 €**.
- **Suppression progressive maintenue jusqu'en 2030** (report).
- **Taux maximal : 0,28 % pour 2026** (et 2027), puis 0,19 % en 2028, 0,09 % en 2029, suppression en 2030.

### Taxe sur les salaires
Due par les employeurs **non assujettis à la TVA** (ou assujettis à moins de 90 %) : associations, certaines activités exonérées, secteur médical/financier… Barème progressif par tranches sur les rémunérations. → Vérifier l'assujettissement du dossier.

### Taxes sur l'affectation des véhicules (ex-TVS)
Pour les sociétés utilisant des véhicules de tourisme : **taxe annuelle sur les émissions de CO₂** + **taxe sur les polluants atmosphériques** (ancien système malus/ancienneté). Déclarées annuellement.

---

## 7. Échéances fiscales importantes (mémo)
| Échéance | Quand |
|---|---|
| TVA CA3 (réel normal) | Mensuelle (selon planning, souvent autour du 15-24) |
| Acomptes TVA RSI | Juillet et décembre |
| CA12 (RSI) | 2e jour ouvré après le 1er mai |
| Acomptes IS | 15/03, 15/06, 15/09, 15/12 |
| Solde IS | 15/05 (clôture 31/12) |
| Liasse fiscale | Début mai (clôture 31/12) |
| CFE | 15 décembre |
| CVAE (si due) | Déclaration et solde en mai ; acompte(s) en cours d'année |

---

## ✅ Checklist « préparation des éléments fiscaux » (offshore)
- [ ] Régime fiscal du dossier identifié (IR/IS, BIC/BNC, micro/réel)
- [ ] Comptes révisés et justifiés (Module 11)
- [ ] TVA de l'exercice bouclée et cohérente
- [ ] Immobilisations et amortissements à jour
- [ ] Provisions / dépréciations recensées (à valider)
- [ ] Comptes courants d'associés justifiés
- [ ] Éléments de réintégration/déduction signalés (amendes, TVS, etc.) **pour décision EC**
- [ ] Tableaux de liasse pré-remplis
- [ ] Dossier transmis au chef de mission **sans valider fiscalement à sa place**

---

## 🧪 Cas pratique 12.1 — Calcul d'IS
Une SAS (PME éligible au taux réduit) réalise un bénéfice fiscal de **90 000 €**. Calculez l'IS.

### ✔️ Correction 12.1
- 42 500 × 15 % = 6 375 €
- (90 000 − 42 500) × 25 % = 47 500 × 25 % = 11 875 €
- **IS = 18 250 €**

---

## 🧪 Cas pratique 12.2 — Régime applicable
Un consultant indépendant réalise 70 000 € de recettes (prestations intellectuelles). Quel régime fiscal possible ?

### ✔️ Correction 12.2
Activité **BNC**. 70 000 € < **83 600 €** → éligible au **micro-BNC** (abattement 34 %) ou option pour la **déclaration contrôlée (2035)** si charges réelles élevées. **Le choix se décide avec l'expert-comptable** (le collaborateur ne tranche pas seul).

---

## 📝 Évaluation de fin de module 12
1. Différence IR / IS ?
2. Déclarations : 2031, 2035, 2065 correspondent à quoi ?
3. Taux d'IS 2026 (normal et réduit + condition) ?
4. Dates des acomptes IS ?
5. La CVAE est-elle encore due en 2026 ?

**Seuil : 7/10.**
**Corrigé :** 1) IR = bénéfice imposé chez l'exploitant ; IS = la société paie l'impôt. 2) 2031 = BIC, 2035 = BNC, 2065 = IS. 3) 25 % normal ; **15 % sur les 42 500 premiers €** si CA < 10 M€, capital libéré détenu ≥ 75 % par des personnes physiques. 4) 15/03, 15/06, 15/09, 15/12. 5) **oui**, suppression repoussée à 2030 (taux max 0,28 % en 2026).

---

### 📌 Sources officielles 2026
- IS, taux et acomptes — [impots.gouv.fr](https://www.impots.gouv.fr/professionnel/imposition-des-resultats)
- Régimes micro / réel — [economie.gouv.fr](https://www.economie.gouv.fr/entreprises/gerer-sa-micro-entreprise/micro-entreprise-regime-reel-et-regime-de-la-declaration-controlee-les-3-regimes-dimposition-des-entreprises)
- CVAE / CFE — [bofip.impots.gouv.fr](https://bofip.impots.gouv.fr/bofip/839-PGP.html/identifiant=BOI-CVAE-LIQ-10-20251119)
- Loi de finances 2026 — [economie.gouv.fr](https://www.economie.gouv.fr/entreprises/gerer-sa-fiscalite-et-ses-impots/loi-de-finances-2026-ce-qui-change-pour-les-entreprises)
